...
async function main() {
  ...
  try {
    ...
    for (const seller of newSellers) {
      // 既存確認
      const checkResult = await pool.query(
        `SELECT * FROM seller_names WHERE seller_id = $1 AND seller_name = $2`,
        [seller.seller_id, seller.seller_name]
      );

      if (checkResult.rows.length > 0) {
        console.log(`  ℹ️  既存のため スキップ: ${seller.seller_id} => "${seller.seller_name}"`);
      } else {
        // 新規挿入
        const insertResult = await pool.query(
          `INSERT INTO seller_names (seller_id, seller_name) VALUES ($1, $2) RETURNING *`,
          [seller.seller_id, seller.seller_name]
        );
